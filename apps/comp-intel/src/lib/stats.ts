...
export function equalWeightedCountries(rows: Observation[], metric: MetricMode): CountrySlice[] {
  const grouped = groupBy(rows, (o) => o.countryCode);
  const out: CountrySlice[] = [];
  for (const [countryCode, list] of grouped) {
    const values = list.map((o) => metricValue(o, metric)).filter(isPresent);
    out.push({
      countryCode,
      country: list[0]?.country ?? countryCode,
      n: list.length,
      observationMedian: median(values),
      directional: directional(list.length),
    });
  }
  return out.sort((a, b) => (b.observationMedian ?? 0) - (a.observationMedian ?? 0));
}
...
export function topRoles(rows: Observation[], metric: MetricMode, limit = 6): RoleSnap[] {
  const grouped = groupBy(rows, (o) => o.roleFamily);
  const out: RoleSnap[] = [];
  for (const [roleFamily, list] of grouped) {
    const values = list.map((o) => metricValue(o, metric)).filter(isPresent);
    out.push({
      roleFamily,
      n: list.length,
      sources: new Set(list.map((o) => o.sourceName)).size,
      observationMedian: median(values),
    });
  }
...
export function likeForLikeLabel(parts: { payType?: string; country?: string; family?: string; exp?: string; city?: string }): string {
  return [parts.payType, parts.country, parts.family, parts.exp, parts.city].filter(Boolean).join(" · ");
}
